          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
                ...
                <div className="text-[10px] text-slate-400 font-bold">{uploadedFile?.size} • {uploadedFile?.pages} pages</div>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
          ...
          <div className="max-w-md mx-auto bg-slate-50/50 rounded-xl p-4 border border-slate-150 text-left font-mono text-[9px] text-slate-500 space-y-2 min-h-[140px] flex flex-col justify-end">
            {sendLogs.map((log, idx) => (
              <div key={idx} className="flex gap-2 items-center">
                <span className="text-[#0D9F8C] font-black">✓</span>
                <span>{log}</span>
              </div>
            ))}
            {sendingProgress < 100 && (
              <div className="flex gap-2 items-center text-slate-400 animate-pulse">
                <span className="animate-spin text-[#0D9F8C] font-black">✦</span>
          ...
          <Card className="rounded-2xl border border-slate-200/50 bg-white/60 p-5 shadow-sm space-y-4">
            ...
            {uploadedFile && (
              <div className="border-t border-slate-100 pt-3.5 space-y-1.5 text-[10px] font-semibold text-slate-500">
                ...
                <div>{uploadedFile.size} • {uploadedFile.pages} pages</div>
              </div>
            )}
          </Card>
        </div>
              ...
              {[
                ...
                <div key={idx} className="flex gap-3 text-[10px] font-semibold">
                  ...
                  <div>
                    ...
                    <div className="text-[9px] text-slate-400 font-medium mt-0.5">{log.id} • {log.time}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {selectedDoc && (
            <>
              <DialogHeader className="border-b border-slate-100 pb-4 mb-4">
                ...
                <div className="text-[10px] text-slate-400 font-bold mt-1">ID: {selectedDoc.id} • Secure Cloud Custody Records</div>
              </DialogHeader>
          ...
          {[
            ...
            <div key={session.ip} className="flex justify-between p-4 bg-white text-xs">
              <div>
                ...
                <span className="text-slate-400 font-semibold ml-2">• {session.device}</span>
              </div>
